...
struct Seq08 : Module {
	...
	Seq08() {
		...
		for (int i = 0; i < Seq08::NUM_LIGHTS; i++) {
...
void Seq08::process(const ProcessArgs &args) {
...
struct Seq08Widget : ModuleWidget {
	Seq08Widget(Seq08 * module) {
		...
		addInput(createInput<PJ301MPort>(Vec(6, 50), module, Seq08::MAIN_IN));

		addOutput(createOutput<PJ3410Port>(Vec(91, 46), module, Seq08::MAIN_OUT));

		for (int i = 0; i < Seq08::NUM_LIGHTS; i++) {
			addChild(createLight<SmallLight<GreenLight>>(Vec(60, 95+(i*35)), module, Seq08::LIGHT_STEP_1+i));
			addParam(createParam<RoundBlackKnob>(Vec(10, 85+(i*35)), module, Seq08::PARAM_STEP_1+i));
		}
...
Model *modelSeq08 = createModel<Seq08, Seq08Widget>("Seq08");
